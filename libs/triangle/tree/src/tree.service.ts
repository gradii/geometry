...
import { FormatEmitEvent } from './interface';
import { TreeNode } from './tree-node';
import { isCheckDisabled, isInArray } from './tree-util';
...
export class TreeService implements OnDestroy {
  ...
  triggerEventChange$?: Subject<FormatEmitEvent> | null = new Subject<FormatEmitEvent>();

  /**
   * trigger event
   */
  eventTriggerChanged(): Observable<FormatEmitEvent> {
   ...
   */
  calcSelectedKeys(selectedKeys: string[], nzNodes: TreeNode[], isMulti: boolean = false): void {
   ...
   */
  calcExpandedKeys(expandedKeys: string[], nzNodes: TreeNode[]): void {
    this.expandedNodeList = [];
   ...
   */
  calcCheckedKeys(checkedKeys: string[], nzNodes: TreeNode[], isCheckStrictly: boolean = false): void {
    this.checkedNodeList = [];
    this.halfCheckedNodeList = [];
    const calc = (nodes: TreeNode[]) => {
      nodes.forEach(node => {
   ...
   */
  afterRemove(node: TreeNode, removeSelf: boolean = true): void {
    let index: number;
    // to reset selectedNodeList & expandedNodeList
    const loopNode = (n: TreeNode) => {
   ...
   * dragNode
   */
  formatEvent(eventName: string, node?: TreeNode, event?: MouseEvent | DragEvent): FormatEmitEvent {
    const emitStructure = {
      'eventName': eventName,
      'node'     : node || undefined,
