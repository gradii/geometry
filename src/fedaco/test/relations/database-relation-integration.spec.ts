...
describe('test database eloquent integration', () => {
  ...
  it('pluck with column name containing a space', () => {
    ...
    var simple = EloquentTestUserWithSpaceInColumnName.oldest('id').pluck(
  ...
  it('basic model hydration', () => {
    ...
    user.setConnection('second_connection');
    user.save();
    var models = EloquentTestUser.on('second_connection').fromQuery('SELECT * FROM users WHERE email = ?',
  ...
  it('touching child model respects parent no touching', () => {
    ...
    expect('It is touching model own timestamps in withoutTouching scope, when it should not.').toBeTruthy(
