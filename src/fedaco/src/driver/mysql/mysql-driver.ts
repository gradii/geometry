...
export class MysqlDriver implements Driver {
  ...
  constructor(connection: Connection) {
    ...
    this.database = DriverUtils.buildDriverOptions(
      this.options.replication ? this.options.replication.master : this.options).database;
  ...
  async connect(): Promise<void> {
    ...
    if (this.options.replication) {
      ...
      this.poolCluster.add('MASTER',
        this.createConnectionOptions(this.options, this.options.replication.master));
   ...
   */
  escapeQueryWithParameters(sql: string, parameters: ObjectLiteral,
                            nativeParameters: ObjectLiteral): [string, any[]] {
  ...
  prepareHydratedValue(value: any, columnMetadata: ColumnMetadata): any {
    if (value === null || value === undefined) {
      return columnMetadata.transformer ? ApplyValueTransformers.transformFrom(
        columnMetadata.transformer, value) : value;
    }
  ...
  normalizeIsUnique(column: ColumnMetadata): boolean {
    return column.entityMetadata.indices.some(
      idx => idx.isUnique && idx.columns.length === 1 && idx.columns[0] === column);
  }
   ...
   */
  findChangedColumns(tableColumns: TableColumn[],
                     columnMetadatas: ColumnMetadata[]): ColumnMetadata[] {
    return columnMetadatas.filter(columnMetadata => {
      ...
      const isColumnChanged = tableColumn.name !== columnMetadata.databaseName
        ...
        || (tableColumn.enum && columnMetadata.enum && !OrmUtils.isArraysEqual(tableColumn.enum,
   ...
   */
  protected createConnectionOptions(options: MysqlConnectionOptions,
                                    credentials: MysqlConnectionCredentialsOptions): Promise<any> {
  ...
  private prepareDbConnection(connection: any): any {
    ...
    if (connection.listeners('error').length === 0) {
      connection.on('error',
        (error: any) => logger.log('warn', `MySQL connection raised an error. ${error}`));
    }
    return connection;
  }
   ...
   */
  protected compareDefaultValues(columnMetadataValue: string | undefined,
                                 databaseValue: string | undefined): boolean {
