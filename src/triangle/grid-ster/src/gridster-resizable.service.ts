...
import { GridsterResizeEventType, MouseEvent2 } from './gridster-resize-event-type.interface';
...
export class GridsterResizable {
  ...
  dragStart(e: MouseEvent2): void {
    ...
    if (e.target?.hasAttribute('class') && e.target.getAttribute('class').split(' ').indexOf(
      'handle-n') > -1) {
      this.resizeEventScrollType.n = true;
      this.directionFunction       = this.handleN;
    } else if (e.target?.hasAttribute('class') && e.target.getAttribute('class').split(' ').indexOf(
      'handle-w') > -1) {
