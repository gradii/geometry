...
import {
  DragPlaceholderInsertionEvent,
  DragPlaceholderInsertionIndexEvent
} from './placeholder-insertion-event.type';
...
export class DroppableDirective implements OnInit, AfterViewInit, OnDestroy {
  ...
  @Input() placeholderStyle: any = {
    backgroundColor: ['#6A98E3', `var(--devui-brand-foil, '#859bff')`],
    opacity        : '.4'
  };
  ...
  private findSortableEl(event) {
    ...
    if (!this.sortContainer.contains(moveElement)) {
      if (this.defaultDropPosition === 'before') {
        overElement = {
          index   : 0,
          el      : childEls.length ? childEls[0] : null,
          position: 'before',
          realEl  : moveElement
        };
  ...
  setPlaceholder = () => {
    this.ngZone.runOutsideAngular(() => {
      ...
      this.placeholderRenderEvent.next({
        width : this.dragDropService.dragOffset.width,
        height: this.dragDropService.dragOffset.height
      });
    });
  };
    ...
    if (rect.width === 0 && rect.height === 0 &&
      ...
      if (targetElement.nextElementSibling) {
        const {
                top : realTop,
                left: realLeft
              } = targetElement.nextElementSibling.getBoundingClientRect();
        rect = {
          x   : realLeft,
          y   : realTop,
          top : realTop,
