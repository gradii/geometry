...
async function introspectionAllowedPolicy({oidc}, {introspectionEndpointAuthMethod}, {clientId}) {
  ...
  if (introspectionEndpointAuthMethod === 'none' && clientId !== oidc.client.clientId) {
    return false;
  }
...
async function expiresWithSession(ctx, {scopes}) {
  return !scopes.has('offline_access');
}

async function issueRefreshToken(ctx, client, {scopes}) {
  return client.grantTypeAllowed('refresh_token') && scopes.has('offline_access');
}
...
async function pairwiseIdentifier(ctx, accountId, {sectorIdentifier}) {
...
function AccessTokenFormat(ctx, {resourceServer}) {
  if (resourceServer) {
    return resourceServer.accessTokenFormat || 'opaque';
  }
  return 'opaque';
}

function ClientCredentialsFormat(ctx, {resourceServer}) {
  if (resourceServer) {
    return resourceServer.accessTokenFormat || 'opaque';
  }
  return 'opaque';
...
function AccessTokenTTL(ctx, {resourceServer}, client) {
  shouldChange('ttl.AccessToken', 'define the expiration for AccessToken artifacts');
  if (resourceServer) {
    return resourceServer.accessTokenTTL || 60 * 60; // 1 hour in seconds
  }
...
function ClientCredentialsTTL(ctx, {resourceServer}, client) {
  shouldChange('ttl.ClientCredentials', 'define the expiration for ClientCredentials artifacts');
  if (resourceServer) {
    return resourceServer.accessTokenTTL || 10 * 60; // 10 minutes in seconds
  }
  return 10 * 60; // 10 minutes in seconds
}
...
function RefreshTokenTTL(ctx, token, {applicationType, tokenEndpointAuthMethod}) {
...
async function interactionsUrl(ctx, {uid}) {
...
function rotateRefreshToken({oidc}) {
...
async function loadExistingGrant({oidc}) {
  const grantId = (oidc.result
    && oidc.result.consent
    && oidc.result.consent.grantId) || oidc.session.grantIdFor(oidc.client.clientId);
