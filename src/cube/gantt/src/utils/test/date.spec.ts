...
describe('tiny-date', () => {
  ...
  it('support getMilliseconds',
    () => expect(date.getMilliseconds()).toBe(date.value.getMilliseconds()));
  ...
  it('support startOf', () => {
    expect(date.startOfDay().getUnixTime()).toBe(
      new GanttDate('2020-02-02 00:00:00').getUnixTime());
    expect(date.startOfMonth().getUnixTime()).toBe(
      new GanttDate('2020-02-01 00:00:00').getUnixTime());
  });

  it('support endOf', () => {
    expect(date.endOfDay().getUnixTime()).toBe(new GanttDate('2020-02-02 23:59:59').getUnixTime());
    expect(date.endOfMonth().getUnixTime()).toBe(
      new GanttDate('2020-02-29 23:59:59').getUnixTime());
  });
