...
import { JoinClause } from "Illuminate/Database/Eloquent/JoinClause";
import { Closure } from "Closure";
import { Builder } from "Illuminate/Database/Eloquent/Builder";
import { JoinClause } from "Illuminate/Database/Query/JoinClause";
import { Arr } from "Illuminate/Support/Arr";
import { Str } from "Illuminate/Support/Str";
import { InvalidArgumentException } from "InvalidArgumentException";
/*trait*/ export class CanBeOneOfMany {
    /*Determines whether the relationship is one-of-many.*/
    protected isOneOfMany: boolean = false;
    /*The name of the relationship.*/
    protected relationName: string;
    /*The one of many inner join subselect query builder instance.*/
    protected oneOfManySubQuery: Builder | null;
    /*Add constraints for inner join subselect for one of many relationships.*/
    public abstract addOneOfManySubQueryConstraints(query: Builder, column: string | null = null, aggregate: string | null = null);
    ...
    public ofMany(column: string | any[] | null = "id", aggregate: string | Function | null = "MAX", relation: string | null = null) {
        this.isOneOfMany = true;
        this.relationName = relation || this.getDefaultOneOfManyJoinAlias(this.guessRelationship());
        var keyName = this.query.getModel().getKeyName();
        var columns = is_string(columns = column) ? {} : column;
        if (!array_key_exists(keyName, columns)) {
            columns[keyName] = "MAX";
        }
        if (aggregate instanceof Closure) {
            var closure = aggregate;
        }
        for (let [column, aggregate] of Object.entries(columns)) {
            ...
            var subQuery = this.newOneOfManySubQuery(this.getOneOfManySubQuerySelectColumns(), column, aggregate);
    ...
    protected newOneOfManySubQuery(groupBy: string | any[], column: string | null = null, aggregate: string | null = null) {
        var subQuery = this.query.getModel().newQuery();
    ...
    protected addOneOfManyJoinSubQuery(parent: Builder, subQuery: Builder, on: string) {
        parent.beforeQuery((parent: Builder) => {
