...
// import { EventManager } from 'Doctrine/Common';
// import { ArrayStatement, CacheException, ResultCacheStatement } from 'Doctrine/DBAL/Cache';
// import { Connection as DriverConnection, PingableConnection, ServerInfoAwareConnection } from 'Doctrine/DBAL/Driver';
// import { InvalidArgumentException } from 'Doctrine/DBAL/Exception';
// import { AbstractPlatform } from 'Doctrine/DBAL/Platforms';
// import { ExpressionBuilder } from 'Doctrine/DBAL/Query/Expression';
// import { AbstractSchemaManager } from 'Doctrine/DBAL/Schema';
// import { Type } from 'Doctrine/DBAL/Types';
...
export class Connection implements DriverConnection {
  ...
  public connect() {
    ...
    const driverOptions            = this.params['driverOptions'] || [];
    ...
    if (this._eventManager.hasListeners(Events.postConnect)) {
      const eventArgs = new Event.ConnectionEventArgs(this);
  ...
  public delete(tableExpression, identifier, types = []) {
    ...
    const columns = (values = conditions = []);
  ...
  public update(tableExpression, data, identifier, types = []) {
    const columns = (values = conditions = set = []);
    ...
    if (is_string(key(types))) {
      const types = this.extractTypeValues(columns, types);
    }
    const sql =
          'UPDATE ' +
          tableExpression +
  ...
  public executeQuery(query, params = [], types = [], qcp = null) {
    ...
    const connection = this.getWrappedConnection();
    const logger     = this._config.getSQLLogger();
    if (logger) {
    ...
    try {
      if (params) {
        ...
        const stmt = connection.query(query);
      }
    } catch (ex) {
      throw DBALException.driverExceptionDuringQuery(
        this._driver,
  ...
  public executeCacheQuery(query, params, types, qcp) {
    ...
    const connectionParams = this.getParams();
    ...
    const data                  = resultCache.fetch(cacheKey);
    if (data !== false) {
      if (data[realKey] !== undefined) {
        const stmt = new ArrayStatement(data[realKey]);
      } else if (array_key_exists(realKey, data)) {
        const stmt = new ArrayStatement([]);
      }
    }
    if (!(stmt !== undefined)) {
      const stmt = new ResultCacheStatement(
  ...
  public project(query, params, func) {
    const result = [];
    const stmt   = this.executeQuery(query, params);
  ...
  public query() {
    ...
    try {
      const statement = connection.query(...$args);
    } catch (ex) {
  ...
  public executeUpdate(query, params = [], types = []) {
    ...
    try {
      if (params) {
        ...
        const result = stmt.rowCount();
      } else {
        const result = connection.exec(query);
      }
    } catch (ex) {
      throw DBALException.driverExceptionDuringQuery(
        this._driver,
  ...
  public exec(statement: string) {
    ...
    try {
      const result = connection.exec(statement);
    } catch (ex) {
      throw DBALException.driverExceptionDuringQuery(
        this._driver,
        ex,
  ...
  public resolveParams(params: any[], types: number[] | string[]) {
    ...
    if (is_int(key(params))) {
      const typeOffset = array_key_exists(0, types) ? -1 : 0;
      let bindIndex  = 1;
      for (const value of params) {
        const typeIndex = bindIndex + typeOffset;
  ...
  private detectDatabasePlatform() {
    const version = this.getDatabasePlatformVersion();
    if (version !== null) {
  ...
  private getDatabasePlatformVersion() {
    ...
    if (this._conn === null) {
      try {
        ...
        const databaseName      = this.params['dbname'];
  ...
  private addIdentifierCondition(identifier, columns, values, conditions) {
    ...
    for (const [columnName, value] of Object.entries(identifier)) {
      if (value === null) {
  ...
  private extractTypeValues(
    ...
    const typeValues = [];
    for (const [columnIndex, columnName] of Object.entries(columnList)) {
  ...
  private getBindingInfo(value, type) {
    ...
    if (type instanceof Type) {
      const value       = type.convertToDatabaseValue(
        value,
        this.getDatabasePlatform()
      );
      const bindingType = type.getBindingType();
    } else {
