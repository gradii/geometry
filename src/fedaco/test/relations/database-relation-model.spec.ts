...
describe('test database eloquent model', () => {
  ...
  it('update process without timestamps', () => {
    var model        = this.getMockBuilder(EloquentModelEventObjectStub).setMethods(
  ...
  it('from date time milliseconds', () => {
    ...
    var model = this.getMockBuilder('Illuminate\\Tests\\Database\\EloquentDateModelStub').setMethods(
      ['getDateFormat']).getMock();
  ...
  it('insert process', () => {
    var model = this.getMockBuilder(EloquentModelStub).setMethods(
  ...
  it('push one relation', () => {
    var related1 = this.getMockBuilder(EloquentModelStub).setMethods(
      ['newModelQuery', 'updateTimestamps', 'refresh']).getMock();
    var query    = m.mock(Builder);
    query.shouldReceive('insertGetId').once()._with({
      'name': 'related1'
  ...
  it('push many relation', () => {
    ...
    related1.name   = 'related1';
    related1.exists = false;
    var related2    = this.getMockBuilder(EloquentModelStub).setMethods(
      ['newModelQuery', 'updateTimestamps', 'refresh']).getMock();
    var query       = m.mock(Builder);
    query.shouldReceive('insertGetId').once()._with({
      'name': 'related2'
  ...
  it('model observers can be attached to models', () => {
    ...
    events.shouldReceive('listen').once()._with('eloquent.creating: Illuminate\\Tests\\Database\\EloquentModelStub',
      EloquentTestObserverStub + '@creating');
    events.shouldReceive('listen').once()._with('eloquent.saved: Illuminate\\Tests\\Database\\EloquentModelStub',
  ...
  it('without event dispatcher', () => {
    ...
    events.shouldReceive('listen').once()._with('eloquent.creating: Illuminate\\Tests\\Database\\EloquentModelSaveStub',
      EloquentTestObserverStub + '@creating');
    events.shouldReceive('listen').once()._with('eloquent.saved: Illuminate\\Tests\\Database\\EloquentModelSaveStub',
      EloquentTestObserverStub + '@saved');
    ...
    events.shouldReceive('until').once()._with('eloquent.saving: Illuminate\\Tests\\Database\\EloquentModelSaveStub',
      model);
    events.shouldReceive('dispatch').once()._with('eloquent.saved: Illuminate\\Tests\\Database\\EloquentModelSaveStub',
      model);
    model.last_name = 'Otwell';
    model.save();
  ...
  it('get model attribute method throws exception if not relation', () => {
    ...
    this.expectExceptionMessage(
      'Illuminate\\Tests\\Database\\EloquentModelStub::incorrectRelationStub must return a relationship instance.');
    var model = new EloquentModelStub();
  ...
  it('model attribute casting fails on unencodable data', () => {
    ...
    this.expectExceptionMessage(
      'Unable to encode attribute [objectAttribute] for model [Illuminate\\Tests\\Database\\EloquentModelCastingStub] to JSON: Malformed UTF-8 characters, possibly incorrectly encoded.');
    var model             = new EloquentModelCastingStub();
    model.objectAttribute = {
      'foo': 'b\u00F8r'
    };
  ...
  it('string key type preserved', () => {
    var model = this.getMockBuilder(EloquentKeyTypeModelStub).setMethods(
      ['newModelQuery', 'updateTimestamps', 'refresh']).getMock();
    var query = m.mock(Builder);
