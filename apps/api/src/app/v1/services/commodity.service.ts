...
import { Sequelize, QueryTypes } from 'sequelize'; // 引入 Sequelize 实例
...
export class CommodityService {
  ...
  async queryCommodityList(body: any): Promise<any> {
    ...
    `;
    const commodityList: any[] = await this.sequelize.query(queryCommodityListSQL, {
  ...
  async createCommodity(body: any, username: string): Promise<any> {
    ...
    `;
    await this.sequelize.query(createCommoditySQL, { logging: false });
    return {
      code: 200,
      msg : 'Success'
    };
  ...
  async updateCommodity(body: any, username: string) {
    ...
    const updateCommoditySQL = `
      ...
      WHERE
        id = ${id}
    `;
    await this.sequelize.query(updateCommoditySQL, { logging: false });
    return {
      code: 200,
      msg : 'Success'
    };
  ...
  async deleteCommodity(body: any) {
    ...
    const deleteCommoditySQL = `
      ...
      WHERE
        id = ${id}
    `;
    await this.sequelize.query(deleteCommoditySQL, { logging: false });
    return {
      code: 200,
      msg : 'Success'
    };
