...
export function composeRelease(buildPackage: BuildPackage) {
  ...
  const importAsName = `@gradii/${name}`;

  inlinePackageMetadataFiles(packageOut);

  // Copy all d.ts and metadata files to the `typings/` directory
  copyFiles(packageOut, '**/*.+(d.ts|metadata.json)', join(releasePath, '_typings'));

  // Copy UMD bundles.
  copyFiles(bundlesDir, `${name}?(-*).umd?(.min).js?(.map)`, join(releasePath, '_bundles'));

  // Copy ES5 bundles.
  copyFiles(bundlesDir, `${name}.es5.js?(.map)`, join(releasePath, '_esm5'));
  copyFiles(join(bundlesDir, name), `*.es5.js?(.map)`, join(releasePath, '_esm5'));

  // Copy ES2015 bundles
  copyFiles(bundlesDir, `${name}.js?(.map)`, join(releasePath, '_esm2015'));
  copyFiles(join(bundlesDir, name), `!(*.es5|*.umd).js?(.map)`, join(releasePath, '_esm2015'));
  ...
  createTypingsReexportFile(releasePath, './_typings/index', name);
  createMetadataReexportFile(releasePath, './_typings/index', name, importAsName);
...
function createFilesForSecondaryEntryPoint(buildPackage: BuildPackage, releasePath: string) {
  ...
  buildPackage.secondaryEntryPoints.forEach(entryPointName => {
    ...
    const importAsName = `@gradii/${name}/${entryPointName}`;
    ...
    copyFiles(
      join(packageOut, 'src', entryPointName),
      `**/*.+(d.ts|metadata.json)`,
      join(entryPointDir, '_typings'));

    // Create a typings and a metadata re-export within the entry-point to point to the
    // typings we just copied.
    createTypingsReexportFile(entryPointDir, `./_typings/index`, 'index');
    createMetadataReexportFile(entryPointDir, `./_typings/index`, 'index', importAsName);
