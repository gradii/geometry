...
export class Comparator {
  ...
  public compare(fromSchema, toSchema) {
    ...
    for (let namespace of toSchema.getNamespaces()) {
      if (fromSchema.hasNamespace(namespace)) {
        continue;
      }
      diff.newNamespaces[namespace] = namespace;
    }
    for (let namespace of fromSchema.getNamespaces()) {
      if (toSchema.hasNamespace(namespace)) {
        continue;
      }
      diff.removedNamespaces[namespace] = namespace;
    }
    for (let table of toSchema.getTables()) {
      var tableName = table.getShortestName(toSchema.getName());
      if (!fromSchema.hasTable(tableName)) {
        diff.newTables[tableName] = toSchema.getTable(tableName);
      } else {
        var tableDifferences = this.diffTable(
    ...
    for (let table of fromSchema.getTables()) {
      ...
      var table     = fromSchema.getTable(tableName);
    ...
    for (let [tableName, table] of Object.entries(diff.removedTables)) {
    ...
    for (let sequence of toSchema.getSequences()) {
    ...
    for (let sequence of fromSchema.getSequences()) {
  ...
  public diffTable(table1, table2) {
    ...
    for (let [columnName, column] of Object.entries(table2Columns)) {
    ...
    for (let [columnName, column] of Object.entries(table1Columns)) {
      if (!table2.hasColumn(columnName)) {
        tableDifferences.removedColumns[columnName] = column;
        changes++;
        continue;
      }
      var changedProperties = this.diffColumn(
        column,
    ...
    for (let [indexName, index] of Object.entries(table2Indexes)) {
      if (
    ...
    for (let [indexName, index] of Object.entries(table1Indexes)) {
      if (
      ...
      var table2Index = index.isPrimary()
    ...
    for (let [key1, constraint1] of Object.entries(fromFkeys)) {
      for (let [key2, constraint2] of Object.entries(toFkeys)) {
    ...
    for (let constraint1 of fromFkeys) {
      tableDifferences.removedForeignKeys.push(constraint1);
      changes++;
    }
    for (let constraint2 of toFkeys) {
  ...
  public diffColumn(column1, column2) {
    var properties1       = column1.toArray();
    var properties2       = column2.toArray();
    ...
    if (
      ...
      var length1 = properties1['length'] || 255;
      var length2 = properties2['length'] || 255;
      if (length1 !== length2) {
  ...
  private detectColumnRenamings(tableDifferences) {
    ...
    for (let [addedColumnName, addedColumn] of Object.entries(
      tableDifferences.addedColumns
    )) {
      for (let removedColumn of tableDifferences.removedColumns) {
    ...
    for (let candidateColumns of renameCandidates) {
  ...
  private detectIndexRenamings(tableDifferences) {
    var renameCandidates = [];
    for (let [addedIndexName, addedIndex] of Object.entries(
      tableDifferences.addedIndexes
    )) {
      for (let removedIndex of tableDifferences.removedIndexes) {
    ...
    for (let candidateIndexes of renameCandidates) {
