...
import { OptionComponent, TriOptionSelectionEvent } from '@gradii/triangle/core';
import {
  createKeyboardEvent,
  dispatchEvent,
  dispatchFakeEvent,
...
import { getTriAutocompleteMissingPanelError, TRI_AUTOCOMPLETE_SCROLL_STRATEGY } from '../src/common';
...
describe('autocomplete', () => {
  ...
  describe('panel toggling', () => {
    ...
    it('should close the panel when an option is clicked', fakeAsync(() => {
      ...
      const option = overlayContainerElement.querySelector('tri-option') as HTMLElement;
      option.click();
      fixture.detectChanges();
    ...
    it('should close the panel when a newly created option is clicked', fakeAsync(() => {
      ...
      let options =
        overlayContainerElement.querySelectorAll('tri-option') as NodeListOf<HTMLElement>;
      options[0].click();
      ...
      options = overlayContainerElement.querySelectorAll('tri-option') as NodeListOf<HTMLElement>;
      options[1].click();
      fixture.detectChanges();
    ...
    it('should hide the panel when the options list is empty', fakeAsync(() => {
      ...
      expect(panel.classList)
        .toContain('tri-autocomplete-visible', `Expected panel to start out visible.`);
      ...
      expect(panel.classList)
        .toContain('tri-autocomplete-hidden', `Expected panel to hide itself when empty.`);
    }));
    ...
    it('should toggle the visibility when typing and closing the panel', fakeAsync(() => {
      ...
      expect(overlayContainerElement.querySelector('.tri-autocomplete-panel')!.classList)
  ...
  describe('keyboard events', () => {
    ...
    it('should scroll to active options below the fold', () => {
      ...
      const scrollContainer =
        document.querySelector('.cdk-overlay-pane .tri-autocomplete-panel')!;
      ...
      [1, 2, 3, 4, 5, 6, 7, 8].forEach(() => trigger._handleKeydown(DOWN_ARROW_EVENT));
    ...
    it('should scroll to active options on UP arrow', () => {
      const scrollContainer = document.querySelector('.cdk-overlay-pane .tri-autocomplete-panel')!;

      fixture.componentInstance.trigger._handleKeydown(UP_ARROW_EVENT);
      fixture.detectChanges();

      // Expect option bottom minus the panel height (32*11 - 256 = 96)
      expect(scrollContainer.scrollTop).toEqual(96, `Expected panel to reveal last option.`);
    });
    ...
    it('should not scroll to active options that are fully in the panel', () => {
      ...
      [7, 6, 5, 4, 3, 2, 1].forEach(() => trigger._handleKeydown(UP_ARROW_EVENT));
    ...
    it('should close the panel when tabbing away from a trigger without results', fakeAsync(() => {
      ...
      expect(overlayContainerElement.querySelector('.tri-autocomplete-panel'))
  ...
  describe('option groups', () => {
    ...
    beforeEach(fakeAsync(() => {
      ...
      tick();
      fixture.detectChanges();
      container = document.querySelector('.tri-autocomplete-panel') as HTMLElement;
    }));
    ...
    it('should scroll to active options on UP arrow', fakeAsync(() => {
      ...
      // 32*12 - 256 + 32*3 = 224
      expect(container.scrollTop).toBe(224, 'Expected panel to reveal last option.');
    }));
    ...
    it('should scroll to active options that are above the panel', fakeAsync(() => {
      ...
      expect(container.scrollTop)
        .toBe(32*2, 'Expected panel to scroll up when option is above panel.');
    }));
  });
  ...
  describe('aria', () => {
    ...
    it('should set role of autocomplete panel to listbox', () => {
      ...
      const panel = fixture.debugElement.query(By.css('.tri-autocomplete-panel')).nativeElement;
  ...
  describe('Fallback positions', () => {
    it('should use below positioning by default', fakeAsync(() => {
      ...
      expect(Math.floor(inputBottom) + AutocompleteMarginHeight)
    ...
    it('should align panel properly when filtering in "above" position', fakeAsync(() => {
      ...
      expect(Math.floor(inputTop))
        .toEqual(Math.floor(panelBottom) + AutocompleteMarginHeight, `Expected panel to stay aligned after filtering.`);
    }));
    ...
    it('should fall back to above position when requested if options are added while the panel is open', fakeAsync(() => {
      ...
      expect(Math.floor(panelRect.top))
        .toBe(Math.floor(inputRect.bottom) + AutocompleteMarginHeight,
      ...
      expect(Math.floor(panelRect.bottom) + AutocompleteMarginHeight)
  ...
  describe('Option selection', () => {
    ...
    it('should be able to preselect the first option', fakeAsync(() => {
      ...
      expect(overlayContainerElement.querySelectorAll('tri-option')[0].classList)
        .toContain('tri-option-active', 'Expected first option to be highlighted.');
    }));
    ...
    it('should be able to configure preselecting the first option globally', fakeAsync(() => {
      ...
      TestBed.resetTestingModule();
      fixture = createComponent(SimpleAutocomplete, [
        {provide: TRI_AUTOCOMPLETE_DEFAULT_OPTIONS, useValue: {autoActiveFirstOption: true}}
      ]);
  ...
  describe('panel closing', () => {
    ...
    it('should emit panel close event when selecting an option', () => {
      ...
      expect(closingActionSpy).not.toHaveBeenCalled();
      option.click();
      expect(closingActionSpy).toHaveBeenCalledWith(jasmine.any(TriOptionSelectionEvent));
    });
  ...
  describe('misc', () => {
    ...
    it('should transfer the tri-autocomplete classes to the panel element', fakeAsync(() => {
      ...
      const autocomplete = fixture.debugElement.nativeElement.querySelector('tri-autocomplete');
      const panel = overlayContainerElement.querySelector('.tri-autocomplete-panel')!;
    ...
    it('should remove old classes when the panel class changes', fakeAsync(() => {
      ...
      const classList =
        overlayContainerElement.querySelector('.tri-autocomplete-panel')!.classList;
...
@Component({
  template: `
    ...
    <tri-autocomplete [class]="panelClass" #auto="triAutocomplete" [displayWith]="displayFn"
...
class SimpleAutocomplete implements OnDestroy {
  ...
  @ViewChild(AutocompleteTrigger) trigger: AutocompleteTrigger;
  @ViewChild(TriAutocomplete) panel: TriAutocomplete;
...
@Component({
  template: `
    <input triInput placeholder="Choose" [triAutocomplete]="auto" [formControl]="optionCtrl">
    <tri-autocomplete #auto="triAutocomplete">
      <tri-option *ngFor="let option of filteredOptions | async" [value]="option">
        {{option}}
      </tri-option>
    </tri-autocomplete>
  `
...
@Component({
  template: `
    <input triInput placeholder="State" [triAutocomplete]="auto"
...
@Component({
  template: `
    <input triInput placeholder="State" [triAutocomplete]="auto" [(ngModel)]="selectedState"
...
@Component({
  template: `
    <input triInput placeholder="Number" [triAutocomplete]="auto" [(ngModel)]="selectedNumber">
...
@Component({
  ...
  template       : `
    ...
    <tri-autocomplete #auto="triAutocomplete">
      <tri-option *ngFor="let option of options" [value]="option">{{ option }}</tri-option>
    </tri-autocomplete>
  `
})
...
@Component({
  template: `
    <input placeholder="Choose" [triAutocomplete]="auto" [formControl]="optionCtrl">
...
@Component({
  template: `<input placeholder="Choose" [triAutocomplete]="auto" [formControl]="control">`
})
...
@Component({
  template: `
    ...
    <tri-autocomplete #auto="triAutocomplete" (optionSelected)="optionSelected($event)">
...
class AutocompleteWithSelectEvent {
  ...
  @ViewChild(AutocompleteTrigger) trigger: AutocompleteTrigger;
  @ViewChild(TriAutocomplete) autocomplete: TriAutocomplete;
}
...
@Component({
  template: `
    <input type="number" triInput [triAutocomplete]="auto" [(ngModel)]="selectedValue">

    <tri-autocomplete #auto="triAutocomplete">
      <tri-option *ngFor="let value of values" [value]="value">{{value}}</tri-option>
    </tri-autocomplete>
  `
})
...
class AutocompleteWithDifferentOrigin {
  @ViewChild(AutocompleteTrigger) trigger: AutocompleteTrigger;
  @ViewChild(AutocompleteOrigin) alternateOrigin: AutocompleteOrigin;
...
@Component({
  template: `
    <input autocomplete="changed" [(ngModel)]="value" [triAutocomplete]="auto"/>
    <tri-autocomplete #auto="triAutocomplete"></tri-autocomplete>
  `
})
