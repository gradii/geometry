...
import {InvalidClientMetadata} from './errors';
import sectorIdentifier from './sector_identifier';
import instance from './weak_cache';
import formatters from './formatters';
import pick from './_/pick';
import without from './_/without';
import omitBy from './_/omit_by';
...
export default function getSchema(provider) {
  ...
  const ENUM = {
    ...
    id_token_signed_response_alg: ({response_types}) => {
      if (!response_types.join(' ').includes('id_token')) {
