...
export function mixinInteractsWithPivotTable<T extends Constructor<{}>>(base: T) {
  // @ts-ignore
  return class InteractsWithPivotTable extends base {
    ...
    public toggle(ids: any, touch: boolean = true) {
      ...
      let detach  = array_values(
        array_intersect(this.newPivotQuery().pluck(this.relatedPivotKey).all(),
          array_keys(records)));
      if (count(detach) > 0) {
