...
import { DragSyncDirective } from '@gradii/triangle/dnd';
...
export class DragDropService {
  ...
  dragSyncGroupDirectives: DragSyncDirective[];
  /*预览功能 */
  ...
  enableDraggedCloneNodeFollowMouse() {
    if (!this.dragCloneNode) {
      ...
      targetCanvasArr.forEach((canvas: any, index: number) => {
        canvas.getContext('2d').drawImage(originCanvasArr[index], 0, 0);
      });

      this.ngZone.runOutsideAngular(() => {
        document.addEventListener('dragover', this.followMouse4CloneNode, {
          capture: true,
          passive: true
        });
      });
