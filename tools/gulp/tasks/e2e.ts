...
task('e2e-app:build', sequenceTask(
  ...
  [
    ...
    'triangle-examples:build-release'
  ],
...
task('e2e-app:copy-release', () => {
  ...
  copySync(join(releasesDir, 'triangle'), join(outDir, 'triangle'));
  copySync(join(releasesDir, 'triangle-examples'), join(outDir, 'triangle-examples'));
});
