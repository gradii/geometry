  ...
  print(name: string = 'download', ignoreElementClass?: string) {
    ...
    html2canvas(root, {
      ...
      onclone: (cloneDocument: Document) => {
        ...
        const cloneGanttDom          = cloneDocument.querySelector(
          `.${ganttClass.replace(/\s+/g, '.')}`) as HTMLElement;
        const cloneGanttContainerDom = cloneDocument.querySelector(
          '.gantt-container') as HTMLElement;
        const cloneCalendarOverlay   = cloneDocument.querySelector(
          '.gantt-calendar-overlay-main') as HTMLElement;
        const cloneLinksOverlay      = cloneDocument.querySelector(
          '.gantt-links-overlay-main') as HTMLElement;
