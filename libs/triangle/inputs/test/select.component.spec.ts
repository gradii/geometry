...
describe('tri-select component', () => {
  ...
  describe('default', () => {
    ...
    it('should allowClear work', () => {
      fixture.detectChanges();
      expect(select.nativeElement.classList).not.toContain('tri-select-allow-clear');
      testComponent.allowClear = true;
      fixture.detectChanges();
      expect(select.nativeElement.classList).toContain('tri-select-allow-clear');
    });
    it('should open work', () => {
      fixture.detectChanges();
    ...
    it('should disabled work', fakeAsync(() => {
      ...
      fixture.detectChanges();
      expect(select.nativeElement.classList).not.toContain('tri-select-enabled');
    ...
    it('should keydown origin work', () => {
      ...
      dispatchKeyboardEvent(select.nativeElement.querySelector('.tri-select-selection'), 'keydown', UP_ARROW);
      fixture.detectChanges();
    ...
    it('should blur after user hits enter key in single mode', () => {
      ...
      select.nativeElement.click();
      fixture.detectChanges();
      dispatchKeyboardEvent(select.nativeElement.querySelector('.tri-select-selection'), 'keydown', DOWN_ARROW);
      fixture.detectChanges();
      expect(spy).not.toHaveBeenCalled();
      dispatchKeyboardEvent(select.nativeElement.querySelector('.tri-select-selection'), 'keydown', ENTER);
      fixture.detectChanges();
      expect(spy).toHaveBeenCalled();
    });
    it('should support keydown events to open and close select panel', fakeAsync(() => {
      fixture.detectChanges();
      dispatchKeyboardEvent(select.nativeElement.querySelector('.tri-select-selection'), 'keydown', SPACE);
      fixture.detectChanges();
      flush();
      fixture.detectChanges();
      expect(testComponent.open).toBe(true);
      // #2201, space should not close select panel
      dispatchKeyboardEvent(select.nativeElement.querySelector('.tri-select-selection'), 'keydown', TAB);
      fixture.detectChanges();
      flush();
      fixture.detectChanges();
      expect(testComponent.open).toBe(false);
  ...
  describe('form', () => {
    ...
    it('should set disabled work', fakeAsync(() => {
      ...
      flush();
      fixture.detectChanges();
      expect(select.nativeElement.classList).not.toContain('tri-select-disabled');
  ...
  describe('option change', () => {
    ...
    it('should option change work', () => {
      fixture.detectChanges();
      const changeSpy = spyOn(selectComponent.selectService, 'updateTemplateOption');
      fixture.detectChanges();
...
@Component({
  template: `
    <tri-select [(ngModel)]="selectedValue" [allowClear]="true" [mode]="'tags'">
      <tri-option value="jack" label="Jack"></tri-option>
      <tri-option value="lucy" label="Lucy"></tri-option>
      <tri-option value="disabled" label="Disabled" disabled customContent>Disabled</tri-option>
    </tri-select>
  `
})
  ...
  template: `
    <tri-select>
      ...
      <tri-option-group label="Manager" *ngIf="displayGroup">
        <tri-option value="jack" label="Jack" *ngIf="displayGroupInner"></tri-option>
        <tri-option value="lucy" label="Lucy"></tri-option>
      </tri-option-group>
