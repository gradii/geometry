...
export class NgxGanttFlatComponent extends GanttUpper implements OnInit, OnChanges, OnDestroy {
  ...
  private buildGroupMergedItems(items: GanttItemInternal[]) {
    ...
    items                                    = items.filter((item) => item.start && item.end).sort(
      (a, b) => a.start.getUnixTime() - b.start.getUnixTime());
    items.forEach((item) => {
      let indexOfMergedItems = -1;
      for (let i = 0; i < mergedItems.length; i++) {
        const subItems = mergedItems[i];
        if (item.start.value > subItems[subItems.length - 1].end.addDays(
          this.mergeIntervalDays).value) {
          subItems.push(item);
  ...
  private buildGroupItems() {
    this.groups.forEach((group) => {
      group.versions = this.buildGroupMergedItems(
        group.items.filter((item, index) => index % 2 === 1));
      group.sprints  = this.buildGroupMergedItems(
        group.items.filter((item, index) => index % 2 === 0));
      // 如果没有数据，默认填充一行空行
