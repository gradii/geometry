...
const examplesPath = path.join(packagesDir, 'triangle-examples');
...
task('build-examples-module', () => {
  ...
  for (const sourcePath of matchedFiles) {
    ...
    if (primaryComponent) {
      ...
      const id = convertToDashCase(primaryComponent.component
        .replace('Example', '')
        .replace('Component', ''));
      ...
      const example: ExampleMetadata = {
        ...
        relativePath        : path.relative(examplesPath, sourcePath).replace(/\\/g, '/'),
