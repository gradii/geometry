...
const webPackagePath = join(bazelBinPath, 'src/triangle/npm_package');

/** Destination path where the web package should be copied to. */
const distPath = join(projectDirPath, 'dist/releases/triangle');

fse.ensureDirSync(distPath);

// Build web package output.
exec('yarn -s bazel build //src/triangle:npm_package --config=snapshot');
